...
avg_residual = sum(sum((closest_pt - matches(:,3:4)).^2))/size(matches,1);
disp("Average Residual for image 2:");
disp(avg_residual);

% img1
% for image 2 we can use transpose of fundamental matrix obtained above
...
avg_residual = sum(sum((closest_pt - matches(:,1:2)).^2))/size(matches,1);
disp("Average Residual for image 1:");
disp(avg_residual);
